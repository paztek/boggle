...
**Grille masquée quand le décompte n'est pas actif.** Tant que le chrono d'une manche n'est pas en
cours — à l'arrêt (`arrete`) comme en pause (`suspendu`) — `App` floute la grille (`Board` reçoit
`blurred`, filtre `blur()` piloté par le token `--board-blur`) et superpose un bouton qui couvre
toute la grille : le libellé est centré, mais la zone cliquable est la grille entière. Le même clic
révèle la grille et (re)lance le chrono — sinon la personne qui manipule l'appareil lirait les
lettres pendant que les autres ne jouent pas. Le bouton porte **Démarrer** avant le début et
**Reprendre** en pause (qui reprend le décompte sans le remettre à zéro). Démarrage et reprise n'ont
donc **plus de bouton propre dans le chronomètre** : ils vivent sur la grille ; le chrono ne garde en
pause que « Réinitialiser ». Le flou n'est **pas animé** — la règle « animations sur
